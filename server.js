...
mongo.connect().then(() => {
  ...
  // ✅ Bot error handlers - CRITICAL for debugging
  bot.on('polling_error', (error) => {
    console.error('❌ TELEGRAM BOT POLLING ERROR:', error.code, error.message);
    if (error.code === 'EFATAL') {
      console.error('❌ FATAL ERROR: Bot token might be invalid or revoked!');
  ...
  if (checkTelegramAuth(req.query)) {
    ...
    try {
      ...
      if (user.hasUsername) {
        ...
        res.redirect(`/pages/dashboard.html?name=${user.username}&pic=${encodeURIComponent(user.photo_url || '')}`);
      } else {
        // Special redirect for users without username
        res.redirect(`/pages/nousername_dashboard.html?name=${encodeURIComponent(displayName)}&pic=${encodeURIComponent(user.photo_url || '')}&hasUsername=false&id=${user.id}`);
      }
...
app.get('/nousername_dashboard.html', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'pages', 'nousername_dashboard.html'));
});
...
// ✅ Bulk admin statistics endpoint - returns all stats in one call
app.get('/api/admin/stats', async (req, res) => {
  try {
    console.log('📊 Loading bulk admin statistics...');
    
    // Get all users with their purchases in one query
    ...
    // Process all users' purchases
    users.forEach(user => {
      const purchases = user.purchases || [];
      
      purchases.forEach(pkg => {
        totalPackages++;
        totalVideos += pkg.count || 0;
        
        // Calculate revenue
        if (pkg.price && pkg.price !== 'FREE' && !pkg.isSample) {
          const priceMatch = pkg.price.match(/Rs\s*([\d.]+)/) || pkg.price.match(/([\d.]+)/);
          if (priceMatch) {
            totalRevenue += parseFloat(priceMatch[1] || priceMatch[0]);
          }
        }
      });
    });
    
    console.log('✅ Bulk stats loaded:', {
      totalPackages,
      totalRevenue,
    ...
    console.error('❌ Error loading bulk admin stats:', error);
...
app.get('/api/users', async (req, res) => {
  try {
    const users = await db.collection("users").find({}, { projection: { username: 1, purchases: 1, _id: 0 } }).toArray();
    res.json(users);
  } catch (err) {
...
// ✅ Minimum Video Count Configuration APIs

// Get minimum video count configuration
app.get('/api/min-video-config', async (req, res) => {
  try {
    const minVideoConfig = await db.collection('settings').findOne({ key: 'min_video_config' });
    const defaultConfig = {
      'Mixed': 100,
      'Mom And Son': 100,
      'Rape': 100,
    ...
    let currentConfig = minVideoConfig?.value || defaultConfig;
    
    // Ensure all packages are present, add missing ones with defaults
    Object.keys(defaultConfig).forEach(packageName => {
      if (currentConfig[packageName] === undefined) {
        currentConfig[packageName] = defaultConfig[packageName];
      }
    });
    
    // If we had to add missing packages, save the updated config
    if (!minVideoConfig || Object.keys(currentConfig).length !== Object.keys(minVideoConfig?.value || {}).length) {
      await db.collection('settings').updateOne(
        { key: 'min_video_config' },
        { 
          $set: { 
            value: currentConfig,
            updatedAt: new Date().toISOString()
      ...
      console.log('✅ Updated min video config with missing packages');
    }
    
    res.json(currentConfig);
  } catch (error) {
    console.error('Error fetching min video config:', error);
    res.status(500).json({ error: 'Failed to fetch minimum video configuration' });
  }
});

// Update minimum video count configuration
app.post('/api/min-video-config', async (req, res) => {
  const { packageName, minVideoCount } = req.body;
  
  if (!packageName || minVideoCount === undefined) {
    return res.status(400).json({ error: 'Package name and minimum video count required' });
  }
  
  try {
    // Get current min video config
    const currentConfig = await db.collection('settings').findOne({ key: 'min_video_config' });
    const minVideoConfig = currentConfig?.value || {
      'Mixed': 100,
      'Mom And Son': 100,
      'Rape': 100,
      'SL Leaks': 100,
    ...
    // Update specific package minimum video count
    minVideoConfig[packageName] = parseInt(minVideoCount);
    ...
    res.json({ success: true, packageName, minVideoCount: parseInt(minVideoCount) });
  } catch (error) {
    console.error('Error updating min video config:', error);
    res.status(500).json({ error: 'Failed to update minimum video configuration' });
  }
});
